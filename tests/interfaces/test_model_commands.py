...
def test_suggested_is_the_builds_for_here_and_the_default_model() -> None:
    picks = suggested(WINDOWS)
    ...
    assert picks[-1] is find(DEFAULT_MODEL)
    assert suggested(Machine(None, nvidia=True)) == ()


def test_resolve_keeps_order_drops_repeats_and_names_every_unknown() -> None:
    assert resolve([DEFAULT_MODEL, DEFAULT_MODEL]) == (find(DEFAULT_MODEL),)
    with pytest.raises(KeyError, match="gpt-5, llama-9"):
        resolve(["gpt-5", DEFAULT_MODEL, "llama-9"])
...
def test_the_list_shows_each_state_and_marks_the_picks(tmp_path: Path) -> None:
    ...
    out_console.print(list_table(models, WINDOWS))
    ...
    assert rows[DEFAULT_MODEL][0] == "*"
    assert rows[DEFAULT_MODEL][-1] == "partial"
...
def test_install_over_the_budget_exits_1_before_any_request(
    ...
    result = runner.invoke(app, ["models", "install", "--yes", DEFAULT_MODEL])
...
def test_remove_of_what_is_not_installed_says_so() -> None:
    result = runner.invoke(app, ["models", "remove", DEFAULT_MODEL])

    assert result.exit_code == 0
    assert f"{DEFAULT_MODEL} was not installed" in result.stdout
